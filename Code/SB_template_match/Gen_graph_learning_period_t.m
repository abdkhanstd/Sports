...
graph_data=[];
idx_graph=1;
for Time_to_Skip_Between_Frames=30:10:150
        ...
        for video_num=start_video_processing_from:end_video_processing_from
                ...
                Actual_region{video_num}=uint8(zeros(size(maps{video_num})));
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %                     Filtering Out the found objects                     %
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % Filtering out the regions from region_votes
        Found_Objects_boundaries={};

        for i =1: size(Actual_region,2)

            region_=Actual_region{i};
            reg=bwconncomp(region_);
            s=size(Actual_region{i});

            for j=1:reg.NumObjects
                index_=reg.PixelIdxList{j};
                [x y]=ind2sub(s,index_);
                ...
                x_max=max(x);
                y_max=max(y);
                height=(y_max-y_min)*(height_margin/100);
                width=(x_max-x_min)*(width_margin/100);


                if abs(x_min-width)>0
                    x_min=abs(x_min-width);
                end
                if abs(y_min-height) >0 
                    y_min=abs(y_min-height);
                end

                if x_max+width < size(Actual_region{i},1)
                      x_max=x_max+width;
                end

                if y_max+height < size(Actual_region{i},2)
                    y_max= y_max+height;  
                end
                Found_Objects_boundaries{i}{j}=[x_min x_max y_min y_max];
        ...
        for i =1: size(Actual_region,2)
            ...
            objects=reg.NumObjects;
            %   min(x) max(x) min(y) max(y)
            for j=1:reg.NumObjects
                bounds=ceil(Found_Objects_boundaries{i}{j});
                display_Items{i}{j} = frame_samples{i}(bounds(1):bounds(2),bounds(3):bounds(4));
                display_Items_rgb{i}{j} = frame_samples_rgb{i}(bounds(1):bounds(2),bounds(3):bounds(4),:);
                ...
                if abs(bounds(1)-ocr_h_margin)>0
                    x_min=abs(x_min-width);
                end
                if abs(y_min-height) >0 
                    y_min=abs(y_min-height);
                ...
                display_Items_ocr{i}{j} = frame_samples_rgb{i}(x_min:x_max,y_min:y_max,:);
        ...
        confirm_score_boards={};
        detected_region_coordinates={};
        for img_number=start_video_processing_from:end_video_processing_from

            % selecting SB based on size
            sz=[];
            for num_items=1:size(display_Items_ocr{img_number},2)
                [rr cc dd]=size(display_Items_ocr{img_number}{num_items});
                sz(num_items)=rr*cc;
            end
            val=max(sz);
            idx=find(sz==val);
            confirm_score_boards{img_number}=display_Items_ocr{img_number}{idx(1)};
            detected_region_coordinates{img_number}= Found_Objects_boundaries{img_number}{idx(1)};               
...
graph_data(1,idx_graph)=duration_to_process;
graph_data(2,idx_graph)= mean(data);
graph_data(3,idx_graph)= mean(miss);
graph_data(4,idx_graph)= recall;
