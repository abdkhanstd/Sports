...
for i=start_video_processing_from:end_video_processing_from % end one is not found

    
    if (size(Found_Objects_boundaries{i},2)>0)
        [a b c r r_u]=check_precision(round(Found_Objects_boundaries{i}),Video_Data{i,3});
    else
        a = 0;
        b =0;
        c=0;
